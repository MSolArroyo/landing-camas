...
const navigation = [
  ...
  { name: 'Amoblamientos para Cocina', href: '#' },
  { name: 'Cantacto', href: '#' },
    ...
    <header className="bg-gray-900/60 z-50 fixed w-full">
      <nav aria-label="Global" className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3 lg:px-8">
        ...
        <div className="hidden lg:flex lg:gap-x-6">
          {navigation.map((item) => (
            <a key={item.name} href={item.href} className="text-sm/6 text-white hover:text-[#35adc2] font-normal hover:font-semibold">
              {item.name}
            </a>
          ))}
        </div>
